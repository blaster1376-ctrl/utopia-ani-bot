...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state or not state.startswith("buy_payment_"):
        await update.message.reply_text(
            "ابتدا از بخش خرید ووچر سفارش ثبت کن."
        )
        return
    ...
    cur.execute("""
        ...
        WHERE id = ?
    """, (
        f"Receipt photo file_id: {file_id}",
        order_id,
    ))
    ...
    await update.message.reply_text(
        ...
        "پس از بررسی ادمین، سفارش شما تکمیل می‌شود."
    )
...
async def orders_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("""
        SELECT id, user_id, order_type, amount, amount_toman, status
        FROM orders
        ORDER BY id DESC
        LIMIT 30
    """)
    ...
    for order in orders:
        oid, uid, otype, amount, toman, status = order
...
async def reply_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("User ID صحیح نیست.")
        return
...
def main():
    if not BOT_TOKEN or BOT_TOKEN == "توکن_ربات_را_اینجا_بگذار":
        raise RuntimeError(
            "BOT_TOKEN را تنظیم کن."
        )
    ...
    print("Utopia Ani bot is running...")
